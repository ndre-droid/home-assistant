...
- NFC when the user mentions an NFC tag/chip. LEAVE_WIFI when the user describes leaving home ("wenn ich gehe", "wenn ich das Haus/WLAN verlasse"); set partnerAware=true there unless told otherwise. MANUAL for button/widget triggers or when unclear.
...
- SONOS: deviceId = speaker ip; commands: "play","pause","stop","volume"(params.volume 0-100),"play_uri"(params.uri, optional params.volume; add params.onlyIfIdle="true" when the user says only if nothing is playing / "wenn nichts läuft"),"mute"(params.on "true"/"false"; works for ALL sources incl. TV audio via ARC - use this to silence TV sound on a Beam),"night_mode"(params.on "true"/"false"; Beam/Arc only),"dialog_level"(params.on; speech enhancement, Beam/Arc only). For ambience sounds (birds, whales, rain...) use "play_uri" with params.uri="" — the app asks the user for a stream URL; still set a low volume if requested (leise ≈ 15). For movie/TV scenes on a Beam, consider night_mode at night.
